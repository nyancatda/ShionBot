...
func TelegramMessageProcessing(json Struct.WebHookJson) {
	...
	if find {
		...
		if err != nil {
			...
			go MessagePushAPI.SendMessage("Telegram", "Default", ChatID, Error(strconv.Itoa(UserID), WikiLink), false, 0, "", 0)
			return
		}
		switch json.Message.Chat.Type {
		case "private":
			go MessagePushAPI.SendMessage("Telegram", "Default", ChatID, WikiInfo, false, 0, "", 0)
		case "supergroup":
			MassageID := json.Message.Message_id
			go MessagePushAPI.SendMessage("Telegram", "Group", ChatID, WikiInfo, true, MassageID, "", 0)
		}
	}
}
